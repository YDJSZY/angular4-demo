                            ...
                            <div class="col-sm-8 col-md-8 col-xs-8">
                                <div class="input-group">
                                    ...
                                    <span class="input-group-btn">
                                        <button class="btn btn-success" data-ng-click="_uploadFile('${data.config.title}','${data.config.uploadFunc}',${data.config.fileId})"><i class="fa fa-upload"></i></button>
                                    </span>
                                </div>
                            </div></div></div>`
                }
...
class Crud{
    constructor($scope,toaster){
        this.apiPrefix = "/api/v0/";
        this.defaultDateRangeName = "今天";
        this.objectList = [];
        this.editingObject = {};
        this.filters = {};
        this.editingMode = "";
        ...
        this.hasBlockType = false;
        this._constants = _constants[this.$scope.currentRoute];
        this.storage = this.$scope.getLocalStorage();//utils/rootScopeFn.js//getLocalStorage
        this.currRouteStorage = this.storage[this.$scope.currentRoute] ? this.storage[this.$scope.currentRoute] : this.storage[this.$scope.currentRoute] = {};//获取当前页面的localStorage
        this.constantConfig = angular.extend({},this._constants,this.currRouteStorage);
        this.$scope.$watch("filters",function (newVal,oldVal) {
            if(newVal!=oldVal) {
    ...
    translateResponse(obj,option) {
        ...
        this.runTooltip();
        if(this.hasBlockType) this.$translateResponse(obj,option)
    }
    ...
    _uploadFile(title,uploadFn,fileId) {
    ...
    searchProtoProperties(obj) {
        if(obj.__proto__ && obj.__proto__.constructor!=Crud){
